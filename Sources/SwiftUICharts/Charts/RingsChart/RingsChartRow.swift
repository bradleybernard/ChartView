...
public struct RingsChartRow: View {
    ...
    func forEachRing(geometry: GeometryProxy) -> some View {
        ForEach(0..<self.chartData.data.count, id: \.self) { index in
            innerRow(geometry: geometry, index: index)
...
//            )
//
//            return Ring(ringWidth:scaledWidth, percent: self.chartData.points[index], foregroundColor:self.style.foregroundColor.rotate(for: index), touchLocation: self.touchRadius)
...
//                .animation(Animation.easeIn(duration: 0.5))
    ...
    func innerRow(geometry: GeometryProxy, index: Int) -> some View {
        let scaledWidth = scaleUp ? self.width * 2.0 : self.width
        let normalPadding = (width + spacing) * CGFloat(index)
        let scaledDiff = (scaledWidth - width) / 2.0
        let padding = min(normalPadding - scaledDiff,
